...
def zip(files, dest):
    # given a list of paths, zip
    # those files up into the given zipfile
...
def zip_to(files, dest):
    """
    If the "--tozip zipfile" option is
    present at the start of the command
    line, run this command:
    "zip -j zipfile <list all the files>".
    This will create a zipfile containing
    the files. Just for fun/reassurance,
    also print the command line you are
    going to do first (as shown in lecture).
    (Windows note: windows does not come
    with a program to produce standard .zip
    archives by default, but you can get
    download the free and open zip program
    from [www.info-zip.org](http://www.info-zip.org/).)
    """

    command = 'zip -j {z} {f}'.format(z=dest, f=' '.join(files))
    # this became a tuple IDK why?
    # print( 'Command I\'m going to try:' + '\n' + command )
    print_str = 'Command I\'m going to try:' + '\n' + command
    print(print_str)
...
def main():
    ...
    parser.add_argument('dirname', type=str, help='dest for special files')
    ...
    # any required args, the general
    # rule is to print a usage message and exit(1).
